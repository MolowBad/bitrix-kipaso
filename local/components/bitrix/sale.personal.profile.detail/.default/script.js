...
(function () {
	BX.Sale.PersonalProfileComponent.PersonalProfileDetail = {
		init: function () {
			var propertyFileList = document.getElementsByClassName(
				"sale-personal-profile-detail-property-file"
			);
			Array.prototype.forEach.call(propertyFileList, function (propertyFile) {
				var deleteFileElement = propertyFile.getElementsByClassName(
					"profile-property-input-delete-file"
				)[0];
				var inputFile = propertyFile.getElementsByClassName(
					"sale-personal-profile-detail-input-file"
				)[0];
				var labelFileInfo = propertyFile.getElementsByClassName(
					"sale-personal-profile-detail-load-file-info"
				)[0];
				var cancelButton = propertyFile.getElementsByClassName(
					...
					BX.delegate(function (event) {
						if (event.target.files.length > 1) {
							labelFileInfo.innerHTML =
								BX.message("SPPD_FILE_COUNT") + event.target.files.length;
							cancelButton.classList.remove("sale-personal-profile-hide");
						} else if (event.target.files.length == 1) {
							fileName = event.target.files[0].name;
							if (fileName.length > 40) {
								labelFileInfo.innerHTML =
									fileName.substr(0, 9) + "..." + fileName.substr(-9);
							} else {
