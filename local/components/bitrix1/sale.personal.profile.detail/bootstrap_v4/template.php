		...
		<form method="post"  class="col sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
			...
			foreach($arResult["ORDER_PROPS"] as $block)
			{
				if (!empty($block["PROPS"]))
				{
					...
					<?
					// Определим, есть ли в блоке раздельные адресные поля
					$codesInBlock = array();
					foreach ($block["PROPS"] as $p) {
						$c = isset($p["CODE"]) ? mb_strtoupper($p["CODE"]) : '';
						if ($c !== '') { $codesInBlock[$c] = true; }
					}
					$hasSeparateAddress = isset($codesInBlock['CITY']) || isset($codesInBlock['STREET']) || isset($codesInBlock['HOUSE'])
						|| isset($codesInBlock['KORPUS']) || isset($codesInBlock['BUILDING']) || isset($codesInBlock['OFFICE'])
						|| isset($codesInBlock['ZIP']) || isset($codesInBlock['COUNTRY']);
					...
					foreach($block["PROPS"] as $property)
					{
						...
						// Скрыть старое поле "Адрес доставки" (обычно TEXTAREA) если есть отдельные поля адреса
						$propCodeUpper = isset($property['CODE']) ? mb_strtoupper($property['CODE']) : '';
						$propNameUpper = isset($property['NAME']) ? mb_strtoupper($property['NAME']) : '';
						$isLegacyAddress = ($property['TYPE'] === 'TEXTAREA') && (
							$propCodeUpper === 'ADDRESS' || $propCodeUpper === 'DELIVERY_ADDRESS' ||
							mb_strpos($propNameUpper, mb_strtoupper('Адрес доставки')) !== false
						);
						if ($hasSeparateAddress && $isLegacyAddress) {
							continue; // пропускаем вывод этого поля
						}
						?>
							<?
