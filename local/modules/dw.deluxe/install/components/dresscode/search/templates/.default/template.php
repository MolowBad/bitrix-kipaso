...
<?if(!empty($arResult["MENU_SECTIONS"]) && count($arResult["MENU_SECTIONS"]) > 1):?>
	<div id="nextSection">
		...
		<ul>
			<?foreach ($arResult["MENU_SECTIONS"] as $arSection):?>
				<li>
					<a<?if($arSection["SELECTED"] != "Y"):?> href="<?=$arSection["FILTER_LINK"]?>"<?endif;?><?if($arSection["SELECTED"] == "Y"):?> class="selected"<?endif;?>>
						<?=$arSection["NAME"]?> (<?=$arSection["ELEMENTS_COUNT"]?>)
					</a>
				</li>
			<?endforeach;?>
		</ul>
...
<?if(!empty($arResult["ITEMS"]) && count($arResult["ITEMS"]) > 1):?>
	<?
		global $preFilter;
		$preFilter = [ "=ID" => $arResult["ITEMS_ID"] ];
	?>
