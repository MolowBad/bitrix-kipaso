...
if ($this->StartResultCache($arParams["CACHE_TIME"], $cacheID)) {
	...
	if (CCatalogSKU::IsExistOffers($arParams["PRODUCT_ID"])) {
		...
		$arOffers = DwSKU::getSkuByProductID(
			$arParams["PRODUCT_ID"],
			$arResult["IBLOCK_ID"],
			array(
	...
	if (empty($arResult["IMAGES"])) {
		$arResult["IMAGES"][] = array(
			"SMALL_PICTURE" => array("SRC" => SITE_TEMPLATE_PATH . "/images/empty.svg"),
			"LARGE_PICTURE" => array("SRC" => SITE_TEMPLATE_PATH . "/images/empty.svg"),
			"SUPER_LARGE_PICTURE" => array("SRC" => SITE_TEMPLATE_PATH . "/images/empty.svg"),
		);
	}
