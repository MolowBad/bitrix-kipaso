			...
			<div class="delivery-modal-container">
				...
				<div class="h3">
					<?if(!empty($arResult["DELIVERY_ITEMS"])):?>
						<?=GetMessage("DELIVERY_CITY_LABEL")?>
						<?if(!empty($_SESSION["USER_GEO_POSITION"]["city"])):?>
							<?=GetMessage("DELIVERY_CITY_IN_LABEL")?> <span><?=$_SESSION["USER_GEO_POSITION"]["city"]?></span>
						<?endif;?>
					<?else:?>
						<?=GetMessage("DELIVERY_ITEMS_NOT_FOUND_HEADING")?>
					<?endif;?>
				</div>
				<?if(empty($arResult["DELIVERY_ITEMS"])):?>
					<div class="delivery-modal-error-text">
						<?=GetMessage("DELIVERY_ITEMS_NOT_FOUND_TEXT", [
							"#REGION_TITLE#" => !empty($_SESSION["USER_GEO_POSITION"]["city"]) ? $_SESSION["USER_GEO_POSITION"]["city"] : ""
						])?>
					</div>
				<?endif;?>
				<?if(!empty($arResult["DELIVERY_ITEMS"])):?>
