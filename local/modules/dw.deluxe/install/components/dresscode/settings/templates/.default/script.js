$(function () {
	...
	var switcherSave = function (event) {
		//j vars
		$switcherItems = $txSwitcher.find(
			".switchByLink, .switcherFile, .switcherSelect, .switcherCheckboxItems, .switcherInputText, .switcherTextarea, .switcherInputTextB64"
		);
		...
		$switcherItems.each(function (ix, nextItem) {
			...
			if (typeof nextItemSettingId != "undefined" && nextItemSettingId != "") {
				...
				if ($nextItem.hasClass("switchByLink")) {
					nextItemSettingValue = $nextItem
						.find(".switchByLinkItem.selected a")
						.data("value");
				}
				...
				//textarea
				else if ($nextItem.hasClass("switcherTextarea")) {
					var $textInput = $nextItem.find("textarea");
					if (checkVar($textInput.val())) {
						nextItemSettingValue = $textInput.val();
					}
				}
			...
			if (nextItemSettingValue != "" && nextItemSettingId != "") {
				...
				if (
					!nextItemIsFile &&
					typeof nextItemSettingValue == "object" &&
					nextItemSettingValue !== null
				) {
		...
		if (checkObject($ajaxContainer) && checkObject(properties)) {
			...
			if (checkVar(propSort) && checkVar(paramName) && checkVar(iblockId)) {
				...
				$.each(properties, function (i, nextItem) {
					...
					//create label
					var $appendLabel = $("<label/>", labelParams).text(
					...
					editParams = {
						...
						class: "settingsEdit",
						href:
							"/bitrix/admin/iblock_edit_property.php?ID=" +
							nextItem["ID"] +
							"&IBLOCK_ID=" +
							iblockId
					};
				...
				if (propIndex > 10) {
					if (checkVar(settingsLang)) {
						//buttons
						var $showAllbutton = $("<a/>", {
							class: "switcherShowAll",
							href: "#"
						}).text(settingsLang["SETTINGS_PROPERTY_SHOW_ALL"]);
						var $hideAllbutton = $("<a/>", {
							class: "switcherHideAll hidden",
							href: "#"
						}).text(settingsLang["SETTINGS_PROPERTY_HIDE_ALL"]);
	...
	var iblockSelectAjax = function ($ajaxContainer, iblockId, propertySort, settingName) {
		//check vars
		if (
			checkObject($ajaxContainer) &&
			checkVar(iblockId) &&
			checkVar(propertySort) &&
			checkVar(settingName)
		) {
			//remove old state
				...
				$.getJSON(settingsAjaxDir + "/ajax.php", ajaxObject, function (jsonData) {
					if (checkVar(jsonData["SUCCESS"]) && jsonData["SUCCESS"] == "Y") {
						...
						if (checkObject(jsonData["PROPERTIES"])) {
							...
							console.log(
								"iblockSelectAjax: properties is empty (iblockId: " + iblockId + ")"
							);
						}
					}
	...
	var iblockSelectHandler = function (event) {
		...
		iblockSelectAjax(
			$ajaxContainerItems,
			iblockId,
			event.data.propertySort,
			event.data.settingName
		);
	...
	var switcherChangeTab = function (event) {
		...
		var date = new Date(new Date().getTime() + 3660 * 1000);
		document.cookie =
			"switcherActiveTabIndex=" + currentIndex + "; path=/; expires=" + date.toUTCString();
	...
	var createWindowTableColumns = function (propertyResult, $containerToAppend) {
		...
		$.each(propertyResult, function (i, nextItem) {
			...
			if (checkVar(nextItem["ERROR"]) && nextItem["ERROR"] == "Y") {
				arColumns.push({
					text: checkVar(nextItem["PROPERTY_ALLREADY_CREATED"])
						? settingsLang["SETTINGS_PROPERTY_ALLREADY_CREATED"]
						: checkVar($nextItem["ERROR_DATA"])
							? $nextItem["ERROR_DATA"]
							: settingsLang["SETTINGS_PROPERTY_ERROR"],
					title: "",
			...
			if (checkObject(arColumns)) {
				...
				$.each(arColumns, function (ix, nextColumn) {
					$newTableRow.append(
						$("<div/>", {
							class: nextColumn["class"],
							title: nextColumn["title"]
						}).text(nextColumn["text"])
					);
				});
		...
		if (checkVar(iblockId)) {
			...
			if (checkVar(settingsAjaxDir)) {
				...
				$.getJSON(settingsAjaxDir + "/ajax.php", ajaxObject, function (jsonData) {
					//check success
					if (
						(checkVar(jsonData["PRODUCT_PROPERTIES"]["SUCCESS"]) &&
							jsonData["PRODUCT_PROPERTIES"]["SUCCESS"] == "Y") ||
						(checkVar(jsonData["SECTION_PROPERTIES"]["SUCCESS"]) &&
							jsonData["SECTION_PROPERTIES"]["SUCCESS"] == "Y")
					) {
		...
		function displayResult(jsonData) {
			...
			if (
				...
				if (checkObject(jsonData["PRODUCT_PROPERTIES"]["PROPERTY_RESULT"])) {
					...
					createWindowTableColumns(
						jsonData["PRODUCT_PROPERTIES"]["PROPERTY_RESULT"],
						$productResultTable
					);
				}
			}
			...
			if (
				...
				if (checkObject(jsonData["SECTION_PROPERTIES"]["PROPERTY_RESULT"])) {
					...
					createWindowTableColumns(
						jsonData["SECTION_PROPERTIES"]["PROPERTY_RESULT"],
						$sectionResultTable
					);
				}
			}
	...
	var СЃreateCustomTheme = function (event) {
		//jquery vars
		var $this = $(this);
		var $createWindow = $this.parents(".txSwitcherWindow");
		var $startContainer = $createWindow.find(".switcherStartContainer");
		...
		//check filling
		if (
			!checkVar(colors["current"]["baseColor"]) ||
			!checkVar(colors["replace"]["baseColor"])
		) {
		...
		//check filling
		if (
			!checkVar(colors["current"]["baseColorHover"]) ||
			!checkVar(colors["replace"]["baseColorHover"])
		) {
		...
		if (
			errors == false &&
			colors["replace"]["baseColorHover"] == colors["replace"]["baseColor"]
		) {
			$baseColorHoverField.addClass("error");
			$baseColorField.addClass("error");
			errors = true;
	...
	$(document).on("click", ".startCreateCustomTheme", СЃreateCustomTheme);
	...
	$(document).on(
		...
		".productIblockSelect",
		{
			containerClass: ".productPropertyAjax",
			propertySort: 5000,
			settingName: "TEMPLATE_PRODUCT_PROPERTIES[]"
		},
		iblockSelectHandler
	);
	$(document).on(
		...
		".skuIblockSelect",
		{
			containerClass: ".skuPropertyAjax",
			propertySort: 100,
			settingName: "TEMPLATE_SKU_PROPERTIES[]"
		},
		iblockSelectHandler
	);
