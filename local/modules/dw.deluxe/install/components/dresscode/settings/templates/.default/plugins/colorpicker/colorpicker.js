	...
	var ColorPicker = (function () {
		var ids = {},
			...
			change = function (ev) {
				...
				cal.data("colorpicker").onChange.apply(cal.data("colorpicker"), [
					col,
					HSBToHex(col),
					HSBToRGB(col)
				]);
			...
			moveIncrement = function (ev) {
				ev.data.field.val(
					Math.max(
						0,
						Math.min(ev.data.max, parseInt(ev.data.val + ev.pageY - ev.data.y, 10))
					)
				);
				if (ev.data.preview) {
			...
			moveSelector = function (ev) {
				change.apply(
					ev.data.cal
						...
						.val(
							parseInt(
								(100 * Math.max(0, Math.min(150, ev.pageX - ev.data.pos.left))) /
									150,
								10
							)
						)
			...
			hide = function (ev) {
				if (!isChildOf(ev.data.cal.get(0), ev.target, ev.data.cal.get(0))) {
					if (
						ev.data.cal.data("colorpicker").onHide.apply(this, [ev.data.cal.get(0)]) !=
						false
					) {
						ev.data.cal.hide();
					}
