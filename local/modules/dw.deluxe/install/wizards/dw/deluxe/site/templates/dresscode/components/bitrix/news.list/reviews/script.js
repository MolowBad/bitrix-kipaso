$(function () {
	var checkReviewForm = function (event) {
		...
		if (!formError) {
			$.getJSON(
				reviewFormAjaxDir + "/ajax.php?act=newReview&" + $thisForm.serialize(),
				function (jsonData) {
					if (jsonData["ERROR"] == "Y") {
						showMessageWindow(
							reviewFormLang["errorHeading"],
							reviewFormLang["errorTypeMessage_" + jsonData["ERROR_TYPE"]]
						);
					} else {
						//metrica
						if (
							typeof globalSettings != "undefined" &&
							typeof globalSettings["TEMPLATE_METRICA_REVIEW_MAGAZINE"] !=
								"undefined" &&
						...
						showMessageWindow(
							reviewFormLang["successHeading"],
							reviewFormLang["successMessage"]
						);
						$thisForm[0].reset();
					}
	...
	$(document).on(
		"click",
		".shop-review-message-window-exit, .shop-review-message-window-close",
		closeReviewWindow
	);
});
