...
<? if (!empty($arResult["ITEMS"])): ?>
	<? if (empty($arParams["AJAX"])): ?>
		<div id="brandList" data-page="1">
			<div class="limiter">
				<a href="<?= SITE_DIR ?>brands/"><span class="heading">
						<?= GetMessage("BRAND_LIST_HEADING") ?>
					</span></a>
				...
				<div class="items">
					<? foreach ($arResult["ITEMS"] as $arElement): ?>
						...
						$picture["src"] = !empty($picture["src"]) ? $picture["src"] : SITE_TEMPLATE_PATH . "/images/empty.svg";
						?>
							...
							<div class="tabloid">
								...
								<a href="<?= $arElement["DETAIL_PAGE_URL"] ?>" class="picture">
									<? if ($arParams["LAZY_LOAD_PICTURES"] == "Y"): ?>
										<img src="<?= SITE_TEMPLATE_PATH ?>/images/lazy.svg" data-lazy="<?= $picture["src"] ?>"
											alt="<?= $arElement["NAME"] ?>" title="<?= $arElement["NAME"] ?>" class="lazy">
									<? else: ?>
										<img src="<?= $picture["src"] ?>" alt="<?= $arElement["NAME"] ?>"
											title="<?= $arElement["NAME"] ?>" class="lazy">
									<? endif; ?>
								</a>
							</div>
						</div>
					...
					<? if (!empty($arResult["HIDE_LAST_ELEMENT"])): ?>
						<div class="item last">
							<a href="#" class="showMore">
								<span class="wp">
									<span class="icon"><img src="<?= SITE_TEMPLATE_PATH ?>/images/showMoreSmall.png"
											alt="<?= GetMessage("BRAND_LIST_SHOW_MORE") ?>"></span>
									<span class="ps">
										<?= GetMessage("BRAND_LIST_SHOW_MORE") ?>
									</span><span class="value">
										<?= $arResult["NEXT_ELEMENTS_COUNT"] ?>
									</span>
