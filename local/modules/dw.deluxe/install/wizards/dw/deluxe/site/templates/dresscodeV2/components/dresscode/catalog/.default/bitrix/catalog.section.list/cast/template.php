	...
	<div id="sectionList">
		<div class="items">
			<? foreach ($arResult["ITEMS"] as $is => $arSection): ?>
				...
				<? $image["src"] = !empty($image["src"]) ? $image["src"] : SITE_TEMPLATE_PATH . "/images/empty.svg"; ?>
				<div class="item">
					<div class="tabloid">
						...
						<? endif; ?>
						<a href="<?= $arSection["SECTION_PAGE_URL"] ?>" class="picture"><img src="<?= $image["src"] ?>"
								alt="<?= $arSection["NAME"] ?>"></a>
						<a href="<?= $arSection["SECTION_PAGE_URL"] ?>" class="name">
							<?= $arSection["NAME"] ?> <span class="count">(
						...
						<? if (!empty($arSection["ELEMENTS"])): ?>
							<div class="innerList">
								<? foreach ($arSection["ELEMENTS"] as $ise => $arSectionElements): ?>
									<div class="element">
										<a href="<?= $arSectionElements["SECTION_PAGE_URL"] ?>" class="name">
											<?= $arSectionElements["NAME"] ?>
										</a>
									</div>
								<? endforeach; ?>
							</div>
