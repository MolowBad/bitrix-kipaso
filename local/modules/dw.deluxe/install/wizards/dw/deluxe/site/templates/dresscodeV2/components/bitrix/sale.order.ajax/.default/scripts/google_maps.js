...
(function () {
	...
	BX.Sale.OrderAjaxComponent.Maps = {
		...
		initializePickUpMap: function (selected) {
			...
			var lat = !!selected
					? parseFloat(BX.util.trim(selected.GPS_N))
					: this.pickUpOptions.defaultMapPosition.lat,
				lng = !!selected
					? parseFloat(BX.util.trim(selected.GPS_S))
					: this.pickUpOptions.defaultMapPosition.lon;
			...
			google.maps.event.addListener(
				this.pickUpMap,
				"click",
				BX.proxy(this.closeAllBalloons, this)
			);
		},
		...
		buildBalloons: function (activeStores) {
			...
			for (i = 0; i < activeStores.length; i++) {
				marker = new google.maps.Marker({
					position: {
						lat: parseFloat(activeStores[i].GPS_N),
						lng: parseFloat(activeStores[i].GPS_S)
					},
					map: this.pickUpMap,
				...
				marker.info = new google.maps.InfoWindow({
					content:
						...
						"<br />" +
						'<a class="btn btn-sm btn-default" data-store="' +
						activeStores[i].ID +
						'">' +
						this.context.params.MESS_SELECT_PICKUP +
						"</a>"
				});
		...
		selectBalloon: function (storeItemId) {
			if (!this.pickUpMap || !this.markers || !this.markers.length) return;
			...
			google.maps.event.addListener(
				...
				BX.delegate(function (e) {
					if (!this.propsMarker) {
						...
						this.propsMarker.addListener(
							"drag",
							BX.proxy(this.onPropsMarkerChanged, this)
						);
						this.propsMarker.addListener(
							"dragend",
							BX.proxy(this.onPropsMarkerChanged, this)
