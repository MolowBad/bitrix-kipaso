...
final class DwSettings
{
	public const DEFAULT_SITE_DIR = "/";

	private static $optionTemplateSettings = [];
	...
	public function __construct(){
		...
		self::$cachePictures = Application::getDocumentRoot() . "/bitrix/upload/dwUploadPicutes";
		self::$settingsPath = Application::getDocumentRoot() . self::$siteDir."settings.php";
		self::$templatePath = Application::getDocumentRoot() . self::$siteTemplatePath;
		self::$sitePath = Application::getDocumentRoot() . self::$siteDir;
	...
	public function getTemplateHeaderPath () :string {

		[ "TEMPLATE_HEADER" => $header ] = $this->getSiteOptions();

		if (empty($header)) {
			$header = "header1";
		}

		return Application::getDocumentRoot() . "/" . SITE_TEMPLATE_PATH."/headers/" . $header . "/template.php";
    ...
    public function getIblocksWithProperty(){
		...
		while($arNextIblock = $rsIblock->Fetch()){
			...
			$arPropertyTypes = $caseIblock == "SKU_IBLOCKS" ? array("L", "E") : [];
	...
	public function getActiveAgreements(): array{
		return \Bitrix\Main\UserConsent\Agreement::getActiveList();
	}
	...
	public function getCurrentSettings(){
		...
		$arSettingsName = [
			...
			"TEMPLATE_AGREEMENT_ID",
			"TEMPLATE_AGREEMENT_PERSONAL_DATA_CATEGORIES",
			"TEMPLATE_PANELS_COLOR",
			"TEMPLATE_HEADER_COLOR",
			"TEMPLATE_METRICA_CODE",
			"TEMPLATE_WORKING_TIME",
			"TEMPLATE_TELEGRAM_LINK",
			"TEMPLATE_WHATSAPP_LINK",
			"TEMPLATE_RUTUBE_LINK",
			"TEMPLATE_DZEN_LINK",
			"TEMPLATE_VIBER_LINK",
			"TEMPLATE_TIKTOK_LINK",
	...
	public function getSettingsFromOption(){
		...
		foreach ($arSites as $arNextSite){
			foreach($arSettingsName as $nextSettingsName){
				if($currentOptionValue = Option::get("dw.deluxe", $nextSettingsName, false, $arNextSite["ID"])){
	...
	public function saveSettings($requestData, $requestFiles){
		...
		if(!empty($requestData)){
			...
			foreach($requestData as $settingName => $settingValue){
				...
				if($settingName === 'TEMPLATE_AGREEMENT_PERSONAL_DATA_CATEGORIES') {
					$settingValue = array_filter(
						array_map(
							fn($row) => htmlspecialcharsbx(trim($row)),
							explode("\n", $settingValue)
						)
					);

					$settingsWrite .= "\t$".$settingName." = ".var_export($settingValue, true).";\r\n";
 		...
 		if(!empty($requestData)){
			...
			foreach($arSettingsName as $nextSettingsName){
				...
				if(!empty($requestData[$nextSettingsName])){
					...
					Option::set("dw.deluxe", $nextSettingsName, $requestData[$nextSettingsName], SITE_ID);
				}
			}
	...
	public static function clearRootFilePath($string){
		if(!empty($string)){
			return str_replace(Application::getDocumentRoot(), "", $string);
		}
	}
