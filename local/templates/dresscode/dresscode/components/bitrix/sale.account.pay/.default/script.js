BX.saleAccountPay = (function () {
	var classDescription = function (params) {
		...
		this.changeInputContainer = this.wrapper.getElementsByClassName(
			"sale-acountpay-fixedpay-container"
		)[0];
	...
	classDescription.prototype.init = function () {
		...
		BX.bindDelegate(
			this.paySystemsContainer,
			"click",
			{ className: "sale-acountpay-pp-company" },
			BX.proxy(function (event) {
				var oldChosenCheckboxList = this.wrapper.querySelectorAll(
					".sale-acountpay-pp-company-checkbox:checked"
				);
		...
		BX.bindDelegate(
			...
			"click",
			{ className: "sale-acountpay-fixedpay-item" },
			BX.proxy(function (event) {
				...
				BX.ajax(
					{
						...
						data: {
							...
							paySystemId: this.wrapper.querySelector(
								".sale-acountpay-pp-company-checkbox:checked"
							).value,
