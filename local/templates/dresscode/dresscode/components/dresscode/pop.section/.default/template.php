...
<? if (!empty($arResult["ITEMS"])): ?>
	<? if (empty($arParams["AJAX"])): ?>
		<div id="popSection" data-page="1">
			<a href="<?= SITE_DIR ?>catalog/"><span class="heading">
					<?= GetMessage("POP_SECTION_HEADING") ?>
				</span></a>
				...
				<? foreach ($arResult["ITEMS"] as $arElement): ?>
					...
					<div class="item" id="<?= $this->GetEditAreaId($arElement["ID"]); ?>">
						<div class="tabloid">
							...
							<div class="nameWrap">
								<a href="<?= $arElement["SECTION_PAGE_URL"] ?>" class="name">
									<?= $arElement["NAME"] ?>
								</a>
							</div>
						</div>
						...
						<a href="#" class="showMore">
							<span class="wp">
								<span class="icon"><img src="<?= SITE_TEMPLATE_PATH ?>/images/showMoreSmall.png"
										alt="<?= GetMessage("POP_SECTION_SHOW_MORE") ?>"></span>
								<span class="ps">
									<?= GetMessage("POP_SECTION_SHOW_MORE") ?>
								</span><span class="value">
									<?= $arResult["NEXT_ELEMENTS_COUNT"] ?>
								</span>
								<span class="small">
									<?= GetMessage("POP_SECTION_SHOWS") ?>
									<?= $arResult["ELEMENTS_COUNT_SHOW"] ?>
									<?= GetMessage("POP_SECTION_FROM") ?>
									<?= $arResult["ITEMS_ALL_COUNT"] ?>
								</span>
							</span>
						</a>
					</div>
