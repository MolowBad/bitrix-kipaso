...
if ($obCache->InitCache(36000000, $USER->GetGroups() . SITE_ID . "v2", $cacheDir)) {
	...
	if (!empty($arResult)) {
		...
		if (CModule::IncludeModule("iblock") && CModule::IncludeModule("catalog")) {
			...
			while ($arProduct = $res->GetNext()) {
				$arMap = array();
				$arProduct["PICTURE"] = !empty($arProduct["DETAIL_PICTURE"]) ? CFile::ResizeImageGet($arProduct["DETAIL_PICTURE"], array('width' => 180, 'height' => 240), BX_RESIZE_IMAGE_PROPORTIONAL, true) : array("src" => SITE_TEMPLATE_PATH . "/images/empty.svg");
